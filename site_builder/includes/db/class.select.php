 ...
 */
 class Select
 {
  ...
  function Select( &$_db, $_query, $try = false )
   {
      ...
      $this->query_id = $_db->query($_query, $try); //echo "j";
  ...
  function addAll(&$ot)
   {
      ...
      for( $i =0 ; $i < $numfields; $i++ )  {
          $ot->addField( $field_name = $this->db->field_name($i,$this->query_id),
                         htmlspecialchars($this->result($field_name), ENT_QUOTES, $encoding = "cp1251"));
      }
   }
  ...
  function addFields($ot,&$fields)
   {   $cnt = count($fields);
       for ( $i = 0; $i < $cnt; $i++ )
          $ot->addField( $fields[$i], htmlspecialchars($this->result($fields[$i]), ENT_QUOTES, $encoding = "cp1251") );
   }
  ...
  function addField($ot,&$field) {
      $ot->addField( $field, htmlspecialchars($this->result($field), ENT_QUOTES, $encoding = "cp1251") );
   //   echo $this->result($field);
  }
