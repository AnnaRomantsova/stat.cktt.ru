...
$shablon = 'front/zakupki/item.html';

$main = &addInCurrentSection($shablon);
...
if ( $id >0 )
   {
      ...
      // echo "select *  from positions pos,plan pl,products pr, grbs g where  g.spz=pl.regNum and pos.plan_id=pl.plan_id and pr.position_id=pos.id and $where ";
       ...
       if ( $r->next_row() ) {
          ...
          $r1 = new Select($db,'select * FROM placingWay  where id = '.$r->result('placingWay'));
          while( $r1->next_row())  {                        
				        $main->addField('placingWay_name',$r1->result('name'));
		      };
          if ($r->result('changeReason') >0 ) {
            $r1 = new Select($db,'select * FROM changeReason  where id = '.$r->result('changeReason'));
             while( $r1->next_row())  {
  
				        $main->addField('changeReason_name',$r1->result('name'));
