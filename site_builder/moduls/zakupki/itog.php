...
// include('functions.php');
 unset($main);
 $FILENAME = $front_html_path.'itog.html';
 $print_FILENAME = $front_html_path.'itog_print.html';
// var_dump($_GET);
...
//echo $query;
 ...
 //ИТОГ
  if ($_GET['no_ident'] || $_GET['okpd_code']) {
        $itog_query =  "select sum(sumPushaseSmallBusiness) as sumPushaseSmallBusiness,
                              sum(sumPushaseRequest) as sumPushaseRequest,
                              sum(sumPushaseSingleSupplier4) as sumPushaseSingleSupplier4,
	                            sum(sumPushaseSingleSupplier5) as sumPushaseSingleSupplier5,
	                            sum(sumContractMaxPrice ) as sumContractMaxPrice,
                              sum(sumPaymentsTotal) as sumPaymentsTotal
                              from (select contractMaxPrice,sum(pr.summax) as summ from  $from $where $groupby ) t";
  } else  {$itog_query = "select sum(sumPushaseSmallBusiness) as sumPushaseSmallBusiness,
                              sum(sumPushaseRequest) as sumPushaseRequest,
                              sum(sumPushaseSingleSupplier4) as sumPushaseSingleSupplier4,
	                            sum(sumPushaseSingleSupplier5) as sumPushaseSingleSupplier5,
	                            sum(sumContractMaxPrice ) as sumContractMaxPrice,
                              sum(sumPaymentsTotal) as sumPaymentsTotal from $from $where $groupby $order"; };
                       
  $r_itog= new Select($db,$itog_query );
  if ( $r_itog->next_row())  {$main->addField('sumPushaseSmallBusiness',round($r_itog->result('sumPushaseSmallBusiness'),2));
                              $main->addField('sumPushaseRequest',round($r_itog->result('sumPushaseRequest'),2));
                              $main->addField('sumPushaseSingleSupplier4',round($r_itog->result('sumPushaseSingleSupplier4'),2));
                              $main->addField('sumPushaseSingleSupplier5',round($r_itog->result('sumPushaseSingleSupplier5'),2));
                              $main->addField('sumContractMaxPrice',round($r_itog->result('sumContractMaxPrice'),2));
                              $main->addField('sumPaymentsTotal',round($r_itog->result('sumPaymentsTotal'),2));                              
                             };   
 //конец ИТОГ
  ...
  while ($r->next_row()) {
        ...
        unset($sub);
        $sub = new outTree();
        $r->addFields($sub,$ar=array('id','cnt_pos','planNumber','versionNumber','name_pbs','sumPaymentsTotal','sumContractMaxPrice','sumPushaseSmallBusiness','sumPushaseSingleSupplier5','sumPushaseSingleSupplier4','sumPushaseRequest'));
