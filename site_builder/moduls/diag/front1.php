...
// $where .= " and year='$year'";
 ...
 $r=new Select($db," select gs.name,(select sum(contractMaxPrice) from positions pos,
                            plan p,grbs g where p.id_pbs=g.id and pos.plan_id=p.plan_id and  p.id_grbs=gs.id $where $where_year)/1000 as summ from grbs_sprav gs  where summ>0 order by summ desc ");
